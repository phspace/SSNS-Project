...
public class KhoaFilter {
    ...
    public double[] movingAverage(double[] data){
        ...
        for (int i=0; i < 6; i++) Output[data.length-6] = Output[data.length-6] +  (data[i+data.length-6])/6;
        for (int i=0; i < 5; i++) Output[data.length-5] = Output[data.length-5] +  (data[i+data.length-5])/5;
